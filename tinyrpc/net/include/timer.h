...
class TimerEvent {
   public:
    int64_t arrive_time;         // 到时时间（毫秒时间戳）
    int64_t interval;            // 定时间隔
    bool is_repeated{false};     // 是否重复执行
    bool is_cancled{false};      // 是否已取消
    ...
    TimerEvent(int64_t interval, bool is_repeated, std::function<void()> task)
        : interval(interval), is_repeated(is_repeated), task(task) {
        arrive_time = getNowMs() + interval;
        DebugLog << "timeevent will occur at " << arrive_time;
    }
    ...
    // 唤醒被取消的定时器
    void wake() { is_cancled = false; }

    // 取消定时器
    void cancle() { is_cancled = true; }

    // 取消重复，将重复定时器转换为一次性定时器
    void cancleRepeated() { is_repeated = false; }
};
 ...
 *
 * 本质上是一个事件文件描述符fd，
 * 其维护了系统关于定时的一个文件描述符timerfd，还有一个事件队列pending_events_
 * timerfd被reactor即epoll维护，每当timerfd到时的时候，就处理pending_events_中到时的事件，即触发它们的回调函数
 */
class Timer : public tinyrpc::FdEvent {
   ...
   public:
    ...
    /**
     * 删除定时事件
     * 从pending_events_中删除指定的定时事件，并标记事件为已取消
     */
    void delTimerEvent(TimerEvent::ptr event);

    /**
     * 重置到期时间
     * 根据pending_events_中最晚的事件重新设置timerfd的到期时间
     * 该方法在添加或删除事件后被调用
     */
