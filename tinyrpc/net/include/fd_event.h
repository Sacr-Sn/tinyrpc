...
/**
 * 继承 std::enable_shared_from_this 的意义就是让你能够在类的成员函数内部使用 shared_from_this()
 * 获取当前对象的共享指针， 确保对象生命周期的正确管理。这样可以防止由于手动管理原始指针或直接使用 this
 * 指针而导致的生命周期问题
 *
 * 该类本质上是对事件描述符fd的封装
 */
...
class FdEventContainer {
   private:
    ...
    std::vector<FdEvent::ptr> fds_;  // 存储shared_ptr<FdEvent>对象，即FdEvent对象的智能指针
