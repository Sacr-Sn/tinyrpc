...
private:
    int m_epfd {-1};  // epoll文件描述符，用于监听所有注册的事件
    int m_wake_fd {-1};  // 唤醒文件描述符(eventfd)，用于跨线程唤醒Reactor
    int m_timer_fd {-1};  // 定时器文件描述符，由Timer管理

    bool m_stop_flag {false};  // 控制事件循环的状态标志
    bool m_is_looping {false};  // 控制事件循环的状态标志
    bool m_is_init_timer {false};

    pid_t m_tid {0};  // Reactor所属线程的ID

    Mutex m_mutex;  // 保护并发访问的互斥锁
    ...
    // 1 -- to add to loop
    // 2 -- to del from loop
    std::map<int, epoll_event> m_pending_add_fds;  // 待添加的文件描述符队列
    std::vector<int> m_pending_del_fds;  // 待删除的文件描述符队列
    std::vector<std::function<void()>> m_pending_tasks;  // 待执行的任务队列

    Timer* m_timer {nullptr};  // 定时器对象指针

    ReactorType m_reactor_type {SubReactor};  // Reactor类型
