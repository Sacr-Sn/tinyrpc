...
int MySQLInstase::reconnect() {
    ...
    DebugLog << "begin to connect mysql{ip:" << m_option.m_addr.getIP() << ", port:" << m_option.m_addr.getPort()
             << ", user:" << m_option.m_user << ", passwd:" << m_option.m_passwd
             << ", select_db: " << m_option.m_select_db << "charset:" << m_option.m_char_set << "}";
    ...
    if (!mysql_real_connect(m_sql_handler, m_option.m_addr.getIP().c_str(), m_option.m_user.c_str(),
                            m_option.m_passwd.c_str(), m_option.m_select_db.c_str(), m_option.m_addr.getPort(), NULL,
                            0)) {
        ErrorLog << "faild to call mysql_real_connect, peer addr[ " << m_option.m_addr.getIP() << ":"
                 << m_option.m_addr.getPort() << "], mysql sys errinfo[" << mysql_error(m_sql_handler) << "]";
        return -1;
    }
...
bool MySQLInstase::isInitSuccess() { return m_init_succ; }
...
int MySQLInstase::query(const std::string& sql) {
    ...
    if (rt != 0) {
        ErrorLog << "excute mysql_real_query error, sql[" << sql << "], mysql sys errinfo["
...
std::string MySQLInstase::getMySQLErrorInfo() { return std::string(mysql_error(m_sql_handler)); }

int MySQLInstase::getMySQLErrno() { return mysql_errno(m_sql_handler); }
