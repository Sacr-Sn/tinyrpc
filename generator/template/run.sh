...
then
  echo "Run tinyrpc error, file: ${PROJECT_BIN_FILE} not exist, please check file"
  exit -1
fi
...
then
  echo "Run tinyrpc error, file: ${PROJECT_CONF_FILE} not exist, please check config file"
  exit -1
fi
...
echo "Start tinyrpc server ${PROJECT_CONF_FILE} success"
